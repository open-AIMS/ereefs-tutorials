...
We will extract the daily mean water temperature for the 10th of October 2022 at 1.5 m depth  across the
...
the month of October 2022 and copy the OPeNDAP data URL.

![](images/thredds_navigation.gif)

```{python serverFileLink}
#| code-overflow: wrap
input_file = "https://thredds.ereefs.aims.gov.au/thredds/dodsC/ereefs/gbr4_v4/daily-monthly/EREEFS_AIMS-CSIRO_gbr4_v4_hydro_daily-monthly-2022-10.nc"
```

We can then open a connection to this file using the `Dataset` function from the `netCDF4` library.

```{python openServerFile}
dailyAggOct22_nc = Dataset(input_file)
```
...
<details><summary><code>dailyAggOct22_nc.title</code></summary>

```{python printFileTitle, echo=FALSE}
dailyAggOct22_nc.title
```
</details>

<details><summary><code>dailyAggOct22_nc.description</code></summary>

```{python printFileDesc, echo=FALSE}
#| code-overflow: wrap
dailyAggOct22_nc.description
```
</details>

<details><summary><code>dailyAggOct22_nc.dimensions</code></summary>

```{python printFileDims, echo=FALSE}
dailyAggOct22_nc.dimensions
```
</details>

<details><summary><code>dailyAggOct22_nc.variables</code></summary>

```{python printFileVars, echo=FALSE}
dailyAggOct22_nc.variables
```
</details>
...
1.5m depth for the 10th of October.

From the `dailyAggOct22.variables` output above we can see that the variable corresponding to temperature
...
Therefore, we need the following:
  ...
  * **file**: an eReefs NetCDF file connection; in our case `dailyAggOct22.nc`.
...
**Time**: Since we have the daily aggregated data for October 2022, and are interested only in a single day
  (the 10th), time is a constant value; i.e. we have a single index for which to extract. From the
  `dailyAggOct22.dimensions` output we can see we have 31 time indexes, these correspond to the day of the
...
```{python extractData}
# EXTRACT DATA
temps_10Oct22_1p5m = dailyAggOct22_nc.variables['temp'][
...
```{python getLonLat}
lons = dailyAggOct22_nc.variables["longitude"][:]
lats = dailyAggOct22_nc.variables["latitude"][:]
```

```{python plotData}
#| fig-cap: Extracted eReefs daily aggregated mean temperature at 1.5m depth for 10 October 2022.
...
plt.contourf(lons, lats, temps_10Oct22_1p5m, 30, transform=ccrs.PlateCarree())
