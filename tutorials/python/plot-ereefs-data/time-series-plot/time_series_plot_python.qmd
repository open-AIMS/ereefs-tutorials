...
The first part of the process is to choose the OPeNDAP end point on the AIMS eReefs THREDDS server. You can view the products [here](https://thredds.ereefs.aims.gov.au/thredds/catalog/ereefs/gbr4_v2/catalog.html). At this stage there is no grouped OPeNDAP service for the entire time series and so this script only works for looking at a single month of data. Hopefully this can be improved in the future.
