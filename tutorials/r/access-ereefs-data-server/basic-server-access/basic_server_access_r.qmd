...
will navigate to the eReefs 4 km Hydrodynamic Model daily aggregated data for the month of October 2022 and
...
```{r openServerFile}
dailyAggOct22.nc <- open.nc(input_file)
```
...
We need to give the function:
  ...
  * `ncfile`: a NetCDF file connection; in our case `dailyAggOct22.nc`.
...
```{r getFileLonLat}
lon <- var.get.nc(dailyAggOct22.nc, "longitude")
lat <- var.get.nc(dailyAggOct22.nc, "latitude")
...
# EXTRACT DATA
temps_10Oct22_1p5m <- var.get.nc(
  ncfile = dailyAggOct22.nc,
  variable = "temp",
  start = start_vector,
  count = count_vector
)
...
```{r createRaster}
temps_raster <- temps_10Oct22_1p5m |>
...
save_file <- "data/ereefsDailyMeanWaterTemperature_10Oct2022_Depth1p5m.nc"
